...
// submit a projects to database admin area
function submit_projects(){
    ...
    if(isset($_POST['submit'])){
        ...
        $sql = "INSERT INTO `projects` (`id`, `cover`, `link`) VALUES (NULL, ?, ?)";
        $stmt = $pdo->prepare($sql);
        $result = $stmt->execute([$cover_id, $link]);
        if($result){
            set_message('success','projects created successfully');
...
function display_projects_admin()
{
    ...
    try{
        ...
        <td class=""><img src="../uploads/thumbnails/{$projects->file_name}" class="br-a" alt="project thumbnail"></td>
    ...
    if (isset($_POST['submit'])) {
        try {
          if(empty($_FILES['cover']['name'])){
            ...
            $sql = "UPDATE `projects` SET `cover` = ? , `link` = ? WHERE `projects`.`id` = ?";
            $update_projects = $pdo->prepare($sql);
            $update_projects->execute([$cover_id, $_POST['link'], $_POST['projects_id']]);
            if ($update_projects) {
